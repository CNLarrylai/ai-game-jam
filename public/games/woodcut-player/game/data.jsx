...
// PoC 数据驱动：小说生成的数据经 index.html 注入 window.GAME_DATA；
// 各字段缺失则回退到下面的内置默认（AI 统治世界），游戏照常跑。
const _GD = (typeof window !== "undefined" && window.GAME_DATA) || {};

// Ambient comments — by scene, picked based on current game state
const SCENE_COMMENTS = _GD.SCENE_COMMENTS || {
  home: [
...
function iconSrc(token){ return null; } /* woodcut: 强制 emoji,不用像素PNG */
...
const INIT_STATS = _GD.INIT_STATS || { sanity: 60, health: 50, hunger: 30, thirst: 30 };

const COMPANIONS_POOL = _GD.COMPANIONS_POOL || [
...
const COMPANIONS = _GD.COMPANIONS || [];

// Destinations for 选择目的地
const DESTINATIONS = _GD.DESTINATIONS || [
...
const LEADERBOARD = _GD.LEADERBOARD || [
...
const GLOBAL_STATS = _GD.GLOBAL_STATS || [
  { v: "1,284", k: "条评论\n影响了本场游戏" },
  { v: "37", k: "个 AI 生成事件 /\n12 物品 / 5 角色" },
  { v: "Day 3", k: "最活跃时刻\nBoss战 86条/分钟" },
];

const WIN_RECAP = _GD.WIN_RECAP || [
...
const SPAM_PHRASES = _GD.SPAM_PHRASES || [
  "开门！开门！开门！",
  "机械狗机械狗机械狗",
  "6666666666",
  "主播快跑啊啊啊啊",
...
const SPAM_USERS = _GD.SPAM_USERS || [
