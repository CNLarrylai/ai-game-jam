...
把一整本小说转成**可玩的、TikTok-Live 直播间形态的互动游戏**（不是纯文字 chat），落进团队三个现成游戏壳之一。**内容与美术、与具体游戏壳解耦**:先理解小说、抽出底座,选机制+选壳,再生成能驱动壳的游戏数据。换书时流程不变,只换少量"按书适配"的旋钮。

## 🎯 主线：小说 → 可玩直播间游戏（端到端）
```
小说 txt → ① 理解(world_bible) → ② 路由(选机制+选壳, SHELL_ROUTER.md)
        → ③ 生成游戏数据(GAME_DATA, LIVE_GAME_SPEC.md) → ④ 注入壳 → 可玩 + 直播间样式
```
- **③ 的产物 = `GAME_DATA`**（资源/物品/同伴/地点/探索格/**弹幕池**/结算），契约见 `references/LIVE_GAME_SPEC.md`——这是让小说变成"能玩 + 兼容直播间展示"的总目标。
- **三个壳**（像素AI生存 / WORLDS LIVE / 最后的人）共享同一数据家族；选哪个 + 各自约束见 `references/SHELL_ROUTER.md`。
- **直播间样式兼容**（70/30布局 + 弹幕采纳/征集 + 数值条 + 投票 + 结算榜 + 刷屏去重）是硬要求，见 LIVE_GAME_SPEC「直播间形态的硬性要求」。
- **Demo 最稳路径**：生存/末世类小说 → 像素壳（题材契合、美术统一、双端直播感最强；PoC 已验证 `public/pixel-poc/`）。

> 下面的"七步流程"是产出 world_bible + 决策卡 + 互动层的**内容深加工**（驱动 CSS 壳的完整代入感版）。若只要快速可玩 Demo，走上面主线①②③④即可（GAME_DATA 一把生成，弹幕池就是直播互动层）。
