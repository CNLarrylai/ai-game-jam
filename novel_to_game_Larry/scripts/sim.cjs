/* sim.cjs — 自测/平衡。以"浏览器方式"加载 gamedata.js + engine.js(提供 window)。
   run: node scripts/sim.cjs [gamesPerPolicy] */
...
function pickChoice(policy, node, res) {
  ...
  for (const c of cs) {
    ...
    const mn = Math.min(r.food, r.health, r.morale), sum = r.food + r.health + r.morale;
...
function stats(policy, donorOn) {
  let won = 0, dead = 0, byCause = { food: 0, health: 0, morale: 0 }, steps = 0, finSum = 0, wonRuns = 0;
  for (let i = 0; i < N; i++) {
    const s = runOne(policy, donorOn);
    steps += s.steps;
    if (s.status === "won") { won++; wonRuns++; finSum += s.res.food + s.res.health + s.res.morale; }
  ...
  return { policy: policy + (donorOn ? "" : " (no-donor)"), win: pct(won), die: pct(dead),
    cause: `饿${(100 * byCause.food / N).toFixed(0)} 疫${(100 * byCause.health / N).toFixed(0)} 丧${(100 * byCause.morale / N).toFixed(0)}`,
    avgSteps: (steps / N).toFixed(1), winSum: wonRuns ? (finSum / wonRuns).toFixed(1) : "-" };
}

console.log(`campaign = ${GAME.campaign.length} nodes | ${N} games/policy`);
...
console.log("\n" + "policy".padEnd(22) + "win".padEnd(6) + "die".padEnd(6) + "death-cause%".padEnd(18) + "steps".padEnd(7) + "winSum");
for (const r of rows) console.log(r.policy.padEnd(22) + r.win.padEnd(6) + r.die.padEnd(6) + r.cause.padEnd(18) + r.avgSteps.padEnd(7) + r.winSum);
