...
function SceneDestination({ D }) {
  useEffectO(() => {
    const t = setTimeout(() => {
      D.adoptComment({ user: "雾", av: "🌫️", text: "废弃医院应该有药！让那栋楼出现在地图上" });
      D.banner({ id: Date.now(), icon: "✨", html: "<b>@雾</b> 的创意生效了！「地图上浮现出一栋废弃医院」" });
    }, 900);
...
function SceneExplore({ D }) {
  ...
  const explore = (t) => {
    ...
    // hex-grid (520×700) is centred in the explore area; convert grid-local → scene coords
    const GRID_OX = (1344 - 520) / 2;        // scene width 1344, grid 520
    const GRID_OY = 70 + (830 - 700) / 2;    // hexwrap top 70 + vertical centring
    const px = GRID_OX + p.left + HEX_W / 2;
    const py = GRID_OY + p.top + HEX_H / 2;
    if (t.type !== "empty") D.spawn({ x: px, y: py });
    ...
    if (t.type === "search") {
      ...
      setTimeout(() => {
        D.banner({ id: Date.now(), big: true, icon: "✨",
          html: "<b>@纸鹤</b> 的创意生效了！「废弃医院里出现了一只机械守卫」" });
      }, 500);
      setTimeout(() => {
        D.story({
          illus: "🤖", source: "@纸鹤",
