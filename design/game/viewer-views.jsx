  ...
  return (
    <div className="enter-page">
      ...
      <div className="enter-hero">
        ...
        <div className="e-meta"><span>主播 <b>{streamerName}</b></span><span>👁 <b>1.2K</b> watching</span><span>❤️ 12.4k</span></div>
...
function RuinsBackdrop() {
  // distant ruined-city silhouette + rubble ground + drifting fog
  const buildings = [
    { l: 30, w: 70, h: 150, lit: [1, 0, 1] }, { l: 110, w: 54, h: 220, lit: [0, 1, 1, 0] },
    { l: 176, w: 80, h: 120, lit: [1, 1] }, { l: 270, w: 60, h: 260, lit: [1, 0, 0, 1] },
    { l: 340, w: 90, h: 180, lit: [0, 1, 0] }, { l: 446, w: 50, h: 240, lit: [1, 1, 0] },
    { l: 510, w: 74, h: 140, lit: [0, 1] }, { l: 598, w: 64, h: 200, lit: [1, 0, 1] },
    { l: 678, w: 96, h: 160, lit: [1, 1, 0, 1] }, { l: 788, w: 56, h: 230, lit: [0, 1, 0] },
    { l: 858, w: 80, h: 130, lit: [1, 0] }, { l: 952, w: 60, h: 200, lit: [0, 1, 1] },
    { l: 1030, w: 78, h: 170, lit: [1, 1] }, { l: 1120, w: 66, h: 250, lit: [0, 1, 0, 1] },
  ];
  return (
    <div className="ruins-bg">
      <Sprite name="moon" size={70} style={{ position: "absolute", right: 120, top: 40, opacity: .8 }} />
      <div className="ruins-skyline">
        {buildings.map((b, i) => (
          <div key={i} className="ruin" style={{ left: b.l, width: b.w, height: b.h }}>
            {b.lit.map((on, j) => (
              <span key={j} className={"rwin " + (on ? "on" : "")}
                style={{ left: 8 + (j % 2) * (b.w / 2), bottom: 18 + Math.floor(j / 2) * 34 }} />
            ))}
          </div>
        ))}
      </div>
      <div className="ruins-ground" />
      <div className="ruins-fog f1" />
      <div className="ruins-fog f2" />
    </div>
  );
}

/* location interior shown when an event fires — "你进入了……" */
function EncounterScene({ name }) {
  const META = {
    pharmacy: { label: "你进入了：废弃药房", cls: "enc-pharmacy" },
    street:   { label: "你进入了：坍塌街区", cls: "enc-street" },
    bunker:   { label: "你进入了：地下弹药库", cls: "enc-bunker" },
  };
  const m = META[name] || META.pharmacy;
  return (
    <div className={"encounter " + m.cls}>
      <div className="enc-label">📍 {m.label}</div>

      {name === "pharmacy" && (
        <React.Fragment>
          <div className="enc-wall" />
          <div className="enc-cross"><span /><span /></div>
          <div className="enc-ceil-light" />
          <div className="enc-shelf left">{[0,1,2,3].map((r) => <div key={r} className="shelf-row">
            {[0,1,2,3].map((c) => <span key={c} className="vial" style={{ background: ["#57e08a","#4dd2ff","#ff4d6d","#ffcf3f"][(r+c)%4] }} />)}
          </div>)}</div>
          <div className="enc-shelf right">{[0,1,2].map((r) => <div key={r} className="shelf-row">
            {[0,1,2,3].map((c) => <span key={c} className="vial" style={{ background: ["#4dd2ff","#ff4d6d","#ffcf3f","#57e08a"][(r+c)%4], opacity: (r+c)%3?1:.3 }} />)}
          </div>)}</div>
          <div className="enc-floor checker" />
          <Sprite name="hero" size={150} className="enc-hero" />
          <Sprite name="crate" size={86} style={{ position: "absolute", left: 160, bottom: "33%" }} />
        </React.Fragment>
      )}

      {name === "street" && (
        <React.Fragment>
          <div className="enc-sky" />
          <div className="enc-cityrow">
            {[0,1,2,3,4,5].map((i) => <div key={i} className="cbuild" style={{ height: 120 + (i*47)%160 }} />)}
          </div>
          <div className="enc-road" />
          <div className="enc-barrel"><div className="fire f-a" /><div className="fire f-b" /><div className="fire f-c" /><div className="firelight" /></div>
          <Sprite name="hero" size={146} className="enc-hero left" />
          <Sprite name="scavenger" size={146} className="enc-hero right flip" />
          <div className="enc-debris d1" /><div className="enc-debris d2" />
        </React.Fragment>
      )}

      {name === "bunker" && (
        <React.Fragment>
          <div className="enc-concrete" />
          <div className="enc-redlight" />
          <div className="enc-stencil">AMMO · 弹药 · 危险</div>
          <div className="enc-floor" />
          <Sprite name="ammo" size={120} style={{ position: "absolute", left: 210, bottom: "33%" }} />
          <Sprite name="ammo" size={92} style={{ position: "absolute", left: 330, bottom: "32%" }} />
          <Sprite name="crate" size={100} style={{ position: "absolute", left: 770, bottom: "33%" }} />
          <Sprite name="ammo" size={80} style={{ position: "absolute", left: 720, bottom: "42%" }} />
          <Sprite name="hero" size={150} className="enc-hero" />
          <div className="enc-pipe p1" /><div className="enc-pipe p2" />
        </React.Fragment>
      )}
    </div>
  );
    ...
    <div className="chat-col">
      <div className="chat-head">
        ...
        <div className="streamer-meta">
          ...
          <div className="viewers">👁 <b>{(viewers / 1000).toFixed(1)}K</b> watching · ❤️ 12.4k</div>
        </div>
      </div>
